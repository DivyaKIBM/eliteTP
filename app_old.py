...
def login():
    try:
        ...
        users = mongo.db.user
       #  emp_details =  mongo.db.employeedetails
       #  dependent_details =  mongo.db.dependent_details
       #  incometax_details =  mongo.db.incometax
       # # ltadetails =  mongo.db.LTA

        user_data = users.find_one({'email': _username})

        emp_details_data = emp_details.find_one({'email': _username})
        empid= emp_details_data['empid']
        emp_name= emp_details_data['empname']
        gender= emp_details_data['gender']
        dependents = emp_details_data['dependents']


        dependent_details_data = dependent_details.find({'empid': empid})


        incometax_details_data = incometax_details.find({'email': _username})
        ltadeclared = incometax_details_data['ltadeclared']
        lta_amount = incometax_details_data['lta_amount']

        # ltadetails_data = ltadetails.find({'empid': empid})
        # ltadeclared= ltadetails_data['lta']
        # lta_amount = ltadetails_data['lta_amount']
